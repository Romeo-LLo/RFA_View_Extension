...
def scale_estimation_4p(q1, q2, q3, q4, d1, d2, d3, mtx):
    ...
    # check
    d1p = np.linalg.norm(ns[0][0] * v1 - ns[1][0] * v2, axis=0)
    d1pp = np.linalg.norm(ns[0][1] * v1 - ns[1][1] * v2, axis=0)

    d2p = np.linalg.norm(ns[1][0] * v2 - ns[2][0] * v3, axis=0)
    d2pp = np.linalg.norm(ns[1][1] * v2 - ns[2][1] * v3, axis=0)

    # d3p = np.linalg.norm(ns[2] * v3 - ns[3] * v4, axis=0)
    ...
    # s4 = ns[3] * scale_1
    p1 = 0.1 * (F + v1 * s1) * np.array([-1, -1, 1])
    p12 = 0.1 * (F + v1 * s11) * np.array([-1, -1, 1])
    p13 = 0.1 * (F + v1 * s2) * np.array([-1, -1, 1])
    p14 = 0.1 * (F + v1 * s22) * np.array([-1, -1, 1])
